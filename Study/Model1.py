...
class ModifiedEncoder(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        memory_output = self.memory_module(hidden_states[:, -1, :])
        hidden_states[:, -1, :] = memory_output  # Replace last hidden state with memory output
        return hidden_states, encoder_outputs.last_hidden_state  # Return both modified and original
...
class ModifiedDecoder(nn.Module):
    ...
    def forward(self, input_ids, encoder_hidden_states, attention_mask, decoder_attention_mask, memory_output):
        ...
        input_embeddings[:, 0, :] += memory_output  # Modify SOH token
...
class CustomBART(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, decoder_input_ids, decoder_attention_mask):
        modified_encoder_hidden_states, original_encoder_hidden_states = self.encoder(input_ids, attention_mask)
        memory_output = modified_encoder_hidden_states[:, -1, :]
        decoder_hidden_states = self.decoder(
            decoder_input_ids, original_encoder_hidden_states, attention_mask, decoder_attention_mask, memory_output
        )
